...
class TFDetect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), w=None):  # TFdetection layer
        ...
        a = tf.convert_to_tensor(anchors).float().view(self.nl, -1, 2)
        self.register_buffer('anchors', a)  # shape(nl,na,2)
        self.register_buffer('anchor_grid', a.clone().view(self.nl, 1, -1, 1, 1, 2))  # shape(nl,1,na,1,1,2)
        self.m = [TFConv2d(x, self.no * self.na, 1, w=w[i]) for i, x in enumerate(ch)]  # output conv
        self.dynamic = False
